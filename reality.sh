...
#所需系统：CentOS 7+、Debian9+、Ubuntu16+
...
#X射线脚本：https://github.com/zxcvos/Xray-script

#X射线核心：https://github.com/XTLS/Xray-core

#现实：https://github.com/XTLS/REALITY

#X射线示例：https://github.com/chika0801/Xray-examples

#Docker cloudflare翘曲：https://github.com/e7h4n/cloudflare-warp

#Cloudflare Warp:https://github.com/haoel/haoel.github.io#943-码头工人-%E4%BB%A3%E7%90%86
...
只读xray_config_manage='/usr/local/etc/xray-script/xray_config_maanage.sh'
...
如果eval类型>/dev/null 2>&1；然后

eval类型“$cmd”>/dev/null 2>&1

elif命令>/dev/null 2>&1；然后
...
[-f“/etc/debian_version”]&&source/etc/os release&&os=“$｛ID｝”&&printf--“%s”“$｛
...
[-f/etc/os发布版]&&awk-f'[=“]'/PRETTY_NAME/｛打印$3、$4、$5｝'/etc/os发布版&&return

[-f/etc/lsb发行版]&&awk-f'[=“]+''/DESCRIPTION/｛print$2｝'/etc/lsb发行版&&return

}

函数_os_ver（）{

local main_ver=“$（echo$（_os_full）|grep-o E”[0-9.]+“）”
...
local check_url=$（echo$1|grep-o E'[^/]+（\.[^/]]+）+\b'|head-n 1）

local check_num=$（echo QUIT|stdbuf-oL openssl s_client-connect“$｛check_url｝:443”-tls1_3-alpn h2 2>&1|grep-Eoi'（TLSv1.3）|（^alpn\s+协议：\s+h2$）|（X25519）'|sort-u|wc-l）

如果[[${check_num}-eq 3]]；然后
...
_error_detect“yum-install-y-epel发布的yum-utils”
...
systemctl-q处于活动状态$｛server_name｝||systemctl-q启动$｛server name｝

systemctl-q已启用$｛server_name｝| | systemctl-q-enable$｛server _name｝

睡眠2

systemctl-q处于活动状态$｛server_name｝&&_info“已启动 $｛服务器名称｝服务“||_error”$｛server_name｝启动失败“
...
systemctl-q处于活动状态$｛server_name｝&&systemctl-q停止$｛server name｝

启用systemctl-q$｛server_name｝&&systemctl-q禁用$｛server name｝

睡眠2

systemctl-q处于活动状态$｛server_name｝||_info“已暂停 $｛服务器名称｝服务“
...
systemctl-q处于活动状态$｛server_name｝&&systemctl-q-restart$｛server name｝||systemctl-q-start$｛服务器名称｝

systemctl-q已启用$｛server_name｝| | systemctl-q-enable$｛server _name｝

睡眠2

systemctl-q处于活动状态$｛server_name｝&&_info“已重启 $｛服务器名称｝服务“||_error”$｛server_name｝启动失败“
...
systemctl-q处于活动状态$｛server_name｝&&systemctl-q-reload$｛server name｝||systemctl-q-start$｛服务器名称｝

systemctl-q已启用$｛server_name｝| | systemctl-q-enable$｛server _name｝

睡眠2

systemctl-q处于活动状态$｛server_name｝&&_info“已重载 $｛服务器名称｝服务“

；；

dr）

_信息“正在重载 systemd配置文件“
...
local data_list=（$（awk-v FS='，''{对于（i=1；i<=NF；i++）arr[i]=$i}END{对于arr中的（i）print arr[i]}'<<<“${1}”）

local index_list=（$（awk-v FS='，''{对于（i=1；i<=NF；i++）arr[i]=$i}END{对于arr中的（i）print arr[i]}'<<<“${2}”）

本地result_list=（）

如果[${#index_list[@]}-ne0]；然后

对于“${index_list[@]}”中的i；做

如果_is_digit“${i}”&&[${i}-ge1]&&[$｛i}-le${#data_list[@]}]；然后
...
local dest_list=（$（jq'.xray.serverNames|key_unsorted'/usr/local/etc/xray script/config.json|grep-Eoi'“.*”'|sed-En's|“（.*）”|\1|p'））
...
本地提示=“请选择你的 dest，当前默认使用 \“$｛cur_dest｝\”，自填选 0:“
...
如果_is_digit“${pick}”||[[“${pick}”-lt 0||“${pik}”-gt${#dest_list[@]}]]；然后
